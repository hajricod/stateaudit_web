        ...
        <div class="card">
            <div class="card-body">
                <div class="d-flex justify-cotent-start align-items-center">
                    ...
                    <h3 class="m-0">{{__('Add FAQ Section')}}</h3>
                </div>
                ...
                <form action="/admin/faq" method="post">
                    ...
                    <div class="form-group">
                        ...
                        <input class="form-control" type="number" name="rank" id="rank" value="{{old('rank')}}">
                        @error('rank')
                    ...
                    <div class="form-group">
                        ...
                        <select name="faq_group_id" id="faq_group_id" class="form-control">
                            ...
                            @foreach ($faqgroups as $faqgroup)
                               <option value="{{$faqgroup->id}}">{{app()->getLocale() == 'ar' ? $faqgroup->title : $faqgroup->title_en}}</option> 
                            @endforeach
                        </select>
                        @error('faq_group_id')
                    ...
                    <div class="form-group text-right" dir="rtl">
                        ...
                        <textarea class="form-control" name="question" id="question" rows="4">{{old('question')}}</textarea>
                        @error('question')
                            <p class="text-danger">{{$message}}</p>
                        @enderror
                    </div>
                    <div class="form-group text-left" dir="ltr">
                        ...
                        <textarea class="form-control" name="question_en" id="question_en" rows="4">{{old('question_en')}}</textarea>
                        @error('question_en')
                            <p class="text-danger">{{$message}}</p>
                        @enderror
                    </div>
                    <div class="group-control pb-3 text-right" dir="rtl">
                        ...
                        <textarea class="content" name="answer" id="content">{{old('answer')}}</textarea>
                        @error('answer')
                            <p class="text-danger"> {{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="group-control pb-3 text-left" dir="ltr">
                        ...
                        <textarea class="content" name="answer_en" id="content_en">{{old('answer_en')}}</textarea>
                        @error('answer_en')
                            <p class="text-danger"> {{ $message }}</p>
                        @enderror
                    </div>

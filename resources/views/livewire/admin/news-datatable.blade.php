<div>
            ...
            <div class="form-group">
                <div class="d-flex">
                    @can('group-media',  App\Models\Group::find(Auth::user()->group_id))
                        <a class="btn btn-primary rounded-circle {{lang() == 'ar' ? 'ml-2' : 'mr-2'}} p-1" href="news/create">
    ...
    <div class="table-responsive text-nowrap">
        <table class="table mb-0 table-hover bg-white border">
            ...
            <tr>
                <th>
                    <a wire:click.prevent="sortBy('image')" role="button" href="#">
                        {{__('Image')}}
                        @include('parts._sort-icon', ['field' => 'image'])
                    </a>
                </th>
                <th>
                    <a wire:click.prevent="sortBy('type')" role="button" href="#">
                        {{__('Type')}}
                        @include('parts._sort-icon', ['field' => 'type'])
                    </a>
                </th>
                <th>
            ...
            <tbody>
                @foreach ($news as $new)
                    <tr scope="row">
                        <td><img src="{{ asset("storage/news/$new->image") }}" alt="{{app()->getLocale() == 'ar' ? $new->title : $new->title_en}}" class="" width="100" height="80"></td>
                        <td>
                            @if (lang() == 'ar')
                                {{$new->type == "news"? 'خبر' : __('Statement')}}
                            @else
                                {{$new->type == "news"? __('News') : __('Statement')}}   
                            @endif
                        </td>
                        <td>{{ Str::limit( app()->getLocale() == 'ar' ? $new->title : $new->title_en, 50)}}</td>
                        <td>{{$new->created_at->format("M j, Y, g:i A")}}</td>
                        <td>
                            <button 
                            class="btn btn-link btnDelete" 
                            data-toggle="modal" 
                            data-target="#deleteModal{{$new->id}}" 
                            data-action="/admin/news/{{$new->id}}">
                            ...
                            <div class="modal fade" id="deleteModal{{$new->id}}" tabindex="-1" role="dialog" aria-hidden="false">
                                <div class="modal-dialog modal-dialog-centered modal-sm" role="document">
                                    <div class="modal-content">
                                        ...
                                        <div class="modal-footer">
                                            ...
                                            <button wire:click="delete({{$new->id}})" type="button" class="btn btn-primary" data-dismiss="modal">
                                                {{__('Yes')}}
                                            </button>
                            ...
                            <a class="btn btn-link" href="/admin/news/{{$new->id}}">
                            ...
                            @can('group-news',  App\Models\Group::find(Auth::user()->group_id))
                                <a class="btn btn-link" href="/admin/news/{{$new->id}}/edit">

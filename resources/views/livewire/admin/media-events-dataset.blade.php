<div>
    <div class="row p-1 d-flex align-items-end">
        <div class="col-md-3">
            <div class="form-group">
                <div class="d-flex">
                    @if (userRole('media.update'))
                        <a class="btn btn-primary rounded-circle mx-2 p-1" href="media_events/create">
    ...
    <div class="table-responsive text-nowrap">
        <table class="table mb-0 table-hover bg-white border">
            ...
            <tr>
                ...
                <th>
                    <a wire:click.prevent="sortBy('title_en')" role="button" href="#">
                        Title
                        @include('parts._sort-icon', ['field' => 'title_en'])
                    </a>
                </th>
                <th>
            ...
            <tbody>
                @foreach ($events as $event)
                    <tr scope="row">
                        ...
                        <td>{{$event->start_on->format("M j, Y")}}</td>
                        <td>{{$event->end_on->format("M j, Y")}}</td>
                        <td>
                            @if (userRole('media.delete'))
                                <button 
                                class="btn btn-link btnDelete" 
                                data-toggle="modal" 
                                data-target="#deleteModal{{$event->id}}" 
                                data-action="/admin/media_events/{{$event->id}}">
                                ...
                                <div class="modal fade" id="deleteModal{{$event->id}}" tabindex="-1" role="dialog" aria-hidden="false">
                                    <div class="modal-dialog modal-dialog-centered modal-sm" role="document">
                                        <div class="modal-content">
                                            ...
                                            <div class="modal-footer">
                                                ...
                                                <button wire:click="delete({{$event->id}})" type="button" class="btn btn-primary" data-dismiss="modal">
                                                    {{__('Yes')}}
                                                </button>
                                ...
                                {{-- <a class="btn btn-link" href="/admin/media_events/{{$event->id}}">
                            ...
                            @if (userRole('media.update'))
                                <a class="btn btn-link" href="/admin/media_events/{{$event->id}}/edit">
    ...
    <div class="d-flex justify-content-between mt-4" dir="{{app()->getLocale() == 'ar'? 'ltr': ''}}">
        ...
        <div>
            {{__('Showing')}} {{$events->firstItem()}} {{__('To')}} {{$events->lastItem()}} {{__('out of')}} {{$events->total()}}
        </div>
    </div>
</div>